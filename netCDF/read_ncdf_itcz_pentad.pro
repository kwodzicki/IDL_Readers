FUNCTION READ_NCDF_ITCZ_PENTAD, in_year, in_week, $
								limit				= limit,       $
								VERBOSE			= verbose,     $
								GETUNITS		= getUnits,    $
...
IF ~KEYWORD_SET(merra) THEN BEGIN
  fname = !ERA_Data+'For_ITCZ_Identify_PENTAD/'+$
    'Pentad_Mean_for_ITCZ_'+STRTRIM(in_year,2)+'.nc'                  ;Read in ERA dataset
ENDIF ELSE BEGIN
...
iid = NCDF_OPEN(fname)                                                ;Open the NCDF file
	...
	time_id = WHERE(INDGEN(73)+1 EQ in_week, CNT)                       ;Find index of data to read in
